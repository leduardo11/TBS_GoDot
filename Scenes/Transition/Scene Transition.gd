...
func change_scene_to_packed(path, delay = 0.1):
	...
	if path is PackedScene:
		...
		# Autoload nodes are already inside the tree, so just clear the previous scene
		var previous_scene = get_tree().current_scene
		if previous_scene != null and previous_scene != path:
			previous_scene.queue_free()
		get_tree().current_scene = null
	else:
		get_tree().change_scene_to_file(str(path))
