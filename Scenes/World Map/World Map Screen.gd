...
# Emitted when Eirika finishes moving
signal eirika_move_completed
...
func move_eirika(eirika_next_position, movement_seconds = 1):
	...
	tween.tween_property($Eirika, "position", eirika_next_position, movement_seconds).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
	...
	tween.finished.connect(set_eirika_idle)
	tween.finished.connect(eirika_move_completed.emit)
...
func move_camera(camera_next_position, movement_seconds = 1):
	var tween = create_tween()
	tween.tween_property($"World Map Cam", "position", camera_next_position, movement_seconds).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
...
func exit():
	...
	var tween = create_tween()
	tween.tween_property($"World Map Music 1", "volume_db", -80.0, 0.5).set_trans(Tween.TRANS_LINEAR).set_ease(Tween.EASE_IN_OUT)
	await $Anim.animation_finished
